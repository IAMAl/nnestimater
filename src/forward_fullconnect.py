...
def forward_fullconnect(\
    ...
    number_of_parallel_kernels  = int(config_prev_layer['number_of_parallel_kernels'])
    output_size                 = int(config_pres_layer['number_of_outputs'])
    total_number_of_multiplies  = number_of_parallel_kernels * output_size * input_size * (1 - zero_ratio_op)
    total_number_of_additions   = (number_of_parallel_kernels * output_size * input_size - 1) * (1 - zero_ratio_op)
    ...
    f_out.writelines("In  Activation     :  {0:20d}".format(int(input_size)))

    f_out.writelines("\n")
    f_out.writelines("Out Activation     :  {0:20d}".format(int(output_size)))

    f_out.writelines("\n")
    f_out.writelines("Param              :  {0:20d}".format(int(total_number_of_params)))
