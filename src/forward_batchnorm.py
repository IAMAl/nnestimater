...
def forward_batchnorm(args, f_out, config_hard, config_prev_layer, config_pres_layer, dim, input_length_): 
    ...
    for no_dim in range(dim):
        ...
        output_length[no_dim]   = int(config_pres_layer['input_dim'+str(no_dim)])
        frame_length[no_dim]    = int(config_prev_layer['input_dim'+str(no_dim)])

        input_size              *= input_length_[no_dim]
        output_size             *= input_length_[no_dim]
    ...
    total_number_of_additions   = (output_size * window_size - 1 + 1) + (number_of_parallel_kernels - 1) * output_size * (1 - zero_ratio_op)
    total_number_of_divisions   = output_size
    ...
    number_of_stores_data                   = output_size
    ...
    for no_dim in range(dim):
        f_out.writelines("{0:9d}, ".format(int(input_length_[no_dim])))
